...
# Clear/blue water can be smaller bodies (>= 25px); green eutrophic lake water bodies must be contiguous (>= 300px)
...
if np.any(is_w_green_lake):
    ...
    if nw > 0:
        counts = np.bincount(lw.ravel())
        v = np.where(counts >= 300)[0]  # real green lakes are large contiguous bodies
        v = v[v != 0]
        if len(v) > 0:
            water_mask |= np.isin(lw, v)
...
# --- 3. BUILT-UP ROOFTOPS (Terracotta & Blue Tin Roofs & High-Texture Buildings) ---
...
is_high_texture_building = ~water_mask & ~veg_mask & (local_std >= 13.0) & (brightness >= 70) & (brightness <= 220)

primary_built = is_terracotta_roof | is_blue_roof | is_high_texture_building

# --- 4. ROADS & TRANSPORTATION ARTERIES ---
# Asphalt roads, highways, avenues, and corridors:
# Neutral chroma, low-to-moderate saturation, |r-g| <= 16, |g-b| <= 36
is_asphalt = (
    ~water_mask & ~veg_mask & ~primary_built &
    (saturation < 0.32) & (chroma < 40) &
    (np.abs(r - g) <= 15) & (np.abs(g - b) <= 36) &
...
total_px = mask.size
print("OPTIMAL CLASSIFICATION BREAKDOWN:")
...
# Top highway center: y: 20-30, x: 150-200
...
Image.fromarray(cmap).save('C:/Users/mohan/.gemini/antigravity-ide/brain/0bc72704-fe0e-4cb6-8d9b-f7bc76149502/optimal_class_map.png')
overlay = (rgb.astype(float) * 0.4 + cmap.astype(float) * 0.6).astype(np.uint8)
Image.fromarray(overlay).save('C:/Users/mohan/.gemini/antigravity-ide/brain/0bc72704-fe0e-4cb6-8d9b-f7bc76149502/optimal_overlay.png')
print("Saved optimal_class_map.png and optimal_overlay.png")
